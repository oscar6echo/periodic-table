...
If electrons behave like waves, physics required a new mathematical equation to describe how these "matter waves" propagate and interact, just as classical equations describe sound or water waves. In wave physics, the simplest wave traveling in one direction is the **plane wave**, written in complex exponential form as:

$$\Psi(x,t)=Ae^{i(kx-\omega t)}$$

where $A$ is the amplitude, $k$ is the wave number (spatial frequency), and $\omega$ is the angular frequency (temporal frequency). This expression will serve as the starting point for translating classical quantities into quantum operators.
...
To understand why this is the case, consider a "free particle" moving through empty space described by the plane wave from Step B. Recall that this wave is characterized by two values:

* **$k$ (Wave Number):** Defined as $k = 2\pi/\lambda$ (measured in radians per meter). It describes how many wave cycles fit into a unit of space.
* **$\omega$ (Angular Frequency):** Defined as $\omega = 2\pi f = 2\pi/T$ (measured in radians per second). It describes how fast the wave oscillates in time.

#### The Role of $2\pi$

Why the $2\pi$? Mathematically, a full cycle of a sine wave or a complex exponential ($e^{i\theta}$) repeats every $2\pi$ radians. By multiplying the spatial frequency ($1/\lambda$) and temporal frequency ($f$) by $2\pi$, we "normalize" the physical units of the wave so they fit perfectly into the geometry of a circle — which is precisely why the plane wave uses the complex exponential form $\Psi(x,t)=Ae^{i(kx-\omega t)}$.

#### Substituting Quantum Values

We can now bridge de Broglie's and Einstein's discoveries into this wave. We use the **reduced Planck constant** ($\hbar = h/(2\pi)$), which conveniently absorbs that $2\pi$ factor:
...
$$\Psi(x,t) = A\exp\!\left[i\!\left(\frac{p}{\hbar}x - \frac{E}{\hbar}t\right)\right] = A\exp\!\left[\frac{i}{\hbar}(px-Et)\right]$$
...
Therefore, the quantum operator for energy is defined as: $E \rightarrow i\hbar\,\partial/\partial t$
...
Therefore, the quantum operator for momentum in one dimension is $-i\hbar\,\partial/\partial x$, which extends to three dimensions using the gradient operator: $p\rightarrow -i\hbar\nabla$.

## 1.3 The General Formula and Its Limits

By substituting these quantum operators back into the classical energy conservation equation ($E = p^2/(2m) + V$) and applying them to the wave function $\Psi$, we arrive at the full, unpacked Time-Dependent Schrödinger Equation:
