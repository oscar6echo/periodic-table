...
$$\frac{d^2R}{dr^2} + \frac{2}{r}\frac{dR}{dr} - \frac{2m}{\hbar^2}[V(r) - E]R = \frac{\ell(\ell+1)}{r^2}R$$
...
**2. At small distances ($r \to 0$):** Dividing by $r^2$ makes the centrifugal term $\frac{\ell(\ell+1)}{r^2}$ grow much faster than the $\frac{1}{r}$ potential or the constant $E$. It dominates everything else, and the equation reduces to:

$$\frac{d}{dr}\left(r^2\frac{dR}{dr}\right) \approx \ell(\ell+1)R$$

Trying $R \sim r^s$ and substituting: the left side becomes $s(s+1)r^s$, so we need $s(s+1) = \ell(\ell+1)$, giving $s = \ell$ or $s = -(\ell+1)$. The second solution diverges at the origin — infinite probability density right at the nucleus is unphysical — so physics keeps $R \sim r^\ell$. For $\ell > 0$ this forces the probability to zero at $r = 0$: the centrifugal barrier acts as a wall preventing collapse into the nucleus. Only $s$-orbitals ($\ell=0$) have a non-zero probability at the origin, which has deep consequences for shielding and penetration (Section 5.5).

**3. The Factoring Strategy:** Knowing both limiting behaviors, we factor them out explicitly:

$$R(r) = r^\ell \cdot v(r) \cdot e^{-kr}$$

By stripping away the known extremes ($r^\ell$ near the origin, $e^{-kr}$ far away), the residual function $v(r)$ is guaranteed to be finite and well-behaved everywhere in between. Substituting this ansatz back into the full radial equation and expanding $v$ as a power series allows us to solve for the exact coefficients — which turn out to be the **Associated Laguerre Polynomials** of Section 5.3.

## 5.3 Solving for $r$: The Laguerre Polynomials

The math leads us to another set of famous functions — this time from the **19th century** — the **Associated Laguerre Polynomials**, $L_{n-\ell-1}^{2\ell+1}(x)$:

$$L_q^p(x) = \frac{e^x x^{-p}}{q!} \frac{d^q}{dx^q} (e^{-x} x^{q+p})$$

For the specific values of $n$ and $\ell$ used in the Hydrogen atom, see the **[Mathematical Annex](../annex/math.md#associated-laguerre-polynomials)**.
...
Just like the particle in a box (Chapter 2), the math only works if we introduce an integer to keep the solution from blowing up to infinity. Remarkably, it is the *very same* integer we first met in the 1-D box of Chapter 2 — there it counted the standing-wave modes allowed between the walls; here it returns in three dimensions as the **Principal Quantum Number**:

**$n$ (The Principal Quantum Number)**

Together with $\ell$ and $m_\ell$ from Chapter 4, $n$ completes the **three spatial quantum numbers** of the hydrogen atom. For the math to stay stable, $n$ must be an integer, and it must always be larger than the azimuthal quantum number $\ell$. This creates the following hierarchy:

* **$n=1$ ($K$ shell):** $\ell=0$ ($s$).  
    1 subshell ($1s$).
* **$n=2$ ($L$ shell):** $\ell=0, 1$ ($s, p$).  
    2 subshells ($2s, 2p$).
* **$n=3$ ($M$ shell):** $\ell=0, 1, 2$ ($s, p, d$).  
    3 subshells ($3s, 3p, 3d$).
* **$n=4$ ($N$ shell):** $\ell=0, 1, 2, 3$ ($s, p, d, f$).  
    4 subshells ($4s, 4p, 4d, 4f$).
* **$n=5$ ($O$ shell):** $\ell=0, 1, 2, 3, 4$ ($s, p, d, f, g$).  
    5 subshells ($5s, 5p, 5d, 5f, 5g$).
* **$n=6$ ($P$ shell):** $\ell=0, \dots, 5$ ($s, p, d, f, g, h$).  
    6 subshells ($6s, 6p, 6d, 6f, 6g, 6h$).
* **$n=7$ ($Q$ shell):** $\ell=0, \dots, 6$ ($s, p, d, f, g, h, i$).  
...
Recall the power-series function $v(r)$ introduced in Section 5.2. For the wave function to stay finite everywhere, that series cannot run on forever — it must **terminate** after a finite number of terms. This termination is possible only for particular values of the energy, and *that* is what quantizes $E$: it is the radial counterpart of the boundary condition $\sin(kL) = 0$ that quantized the particle in a box (Chapter 2).

Carrying this through, we finally derive the exact formula for the energy levels of a Hydrogen-like atom:
...
However, in reality, most atoms have more than one electron. This introduces three critical effects that break this degeneracy:
...
3. **Penetration:** It is a common misconception that subshell shapes ($s, p, d, f$) only affect the *direction* of the electron. In fact, $\ell$ also changes the **radial probability distribution** (the likelihood of finding the electron at a certain distance).
    * **The Centrifugal Barrier:** High angular momentum ($\ell$) acts like an outward "force" that keeps electrons away from the center.
    * **Inner Lobes:** Lower $\ell$ orbitals (especially $s$) have small "peaks" of probability very close to the nucleus (see the "Secondary Peaks" in section 5.6). These "inner lobes" allow the electron to **penetrate** the shielding cloud of inner electrons and feel the full, unshielded pull of the nucleus.
...
The probability of finding the electron between $r$ and $r+dr$ is obtained by integrating $|\psi|^2$ over all angles. Since the spherical harmonics are normalized ($\int |Y_\ell^{m_\ell}|^2\,d\Omega = 1$), the angular part integrates to 1, leaving:
...
Introductory chemistry writes radial probability as $4\pi r^2|\psi|^2$ for $s$-orbitals, where the $4\pi$ is the surface area of a sphere. That shortcut works only for spherically symmetric ($\ell=0$) states.

This formula is the general version. Since $\psi = R_{n\ell}(r)\,Y_\ell^{m_\ell}(\theta,\phi)$ and the spherical harmonics are normalized,

$$\int |\psi|^2\,d\Omega = |R_{n\ell}|^2 \underbrace{\int |Y_\ell^{m_\ell}|^2\,d\Omega}_{=\,1}$$

the angular integral — including the $4\pi$ — is absorbed automatically. What remains, $P(r) = r^2|R_{n\ell}|^2$, holds for **all** orbital types ($s, p, d, f$) without adjustment.
::::
...
| $n$ | $\ell$ | Orbital | Node at $r=0$ | Radial Nodes | Main $r_{mp}$ | Secondary Peaks |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
...
*The $n=5$ row lists only the two extremes — $5s$ (the deepest-penetrating subshell) and $5g$ (the highest angular momentum, with no radial nodes); $5p$, $5d$ and $5f$ follow the same trends seen in their $n=4$ counterparts.*
...
2. **Boundary Condition (Origin):** Angular momentum ($\ell$) creates a "centrifugal barrier" that keeps the electron away from the center.
