...
:::tip Why must it be zero at the walls ($x=0, L$)? 🤔
...
To make the math cleaner, physicists group the constants on the right side into a single squared term, $k^2$ (where $k = \sqrt{2mE}/\hbar$).
...
Earlier, we defined $k^2 = 2mE/\hbar^2$. Now that we know $k = n\pi/L$, we can substitute it back to find the allowed energy ($E$) of the particle:
